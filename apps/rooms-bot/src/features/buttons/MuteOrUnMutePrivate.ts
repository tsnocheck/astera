...
export class MuteOrUnMutePrivate implements IFeature<ButtonInteraction> {
  name = 'muteOrUnMutePrivate';
  ...
  async run({ interaction }: RunFeatureParams<ButtonInteraction>) {
    ...
    const user = new ActionRowBuilder<UserSelectMenuBuilder>().addComponents(
      new UserSelectMenuBuilder()
        .setCustomId('selectMuteOrUnMutePrivate')
        .setPlaceholder('Выберите пользователей')
        .setMaxValues(5)
    )
    await interaction.reply({ content: 'Выберите пользователей для отключения/включения звука.', components: [user], ephemeral: true });
  }
}

export default MuteOrUnMutePrivate;